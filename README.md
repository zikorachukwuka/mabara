...
- **The read-only shell allowlist is narrow.** Only exact whole-word commands (`ls`, `git status`, `git log`, …) with no chaining, redirection, or file-writing flags (`--output`) auto-run; everything else needs your voice. In `--readonly` mode no shell command runs at all.
